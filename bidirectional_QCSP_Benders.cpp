...
bool LB_Same_Direction(IloModel model, IloNum nbs, IloIntArray  nbb, IloNumArray nbQ, IloIntArray  nbreadyT,
	...
	IloNum *ObjVal, IloNum *UB, IloIntArray  nbLocation, BoolMatrix nbprecR, int my, double time_limit_LB);

bool Same_Direction(IloModel model, IloNum nbs, IloIntArray  nbb, IloNumArray nbQ, IloIntArray  nbreadyT,
	BoolVarMatrix yF, IloIntVar CF, IloInt *CF_best, BoolMatrix yF_best,
	IloNum *ObjVal, IloNum *UB, IloIntArray  nbLocation, BoolMatrix nbprecR, int my, double time_limit_UB);
	...
	//	cout << "lazy: " << endl;
		...
		duration_SP += (double)(finish_SP - start_SP) / CLOCKS_PER_SEC;
				...
				for (int j = 1; j < nbTask; j++)
				{
						...
						IloExpr BDcut_Sj(masterEnv);//S_i^T

						int count_Sj = 0;
						int Ctime_Sj = 0;

						for (int n2 = j; n2 < nbTask; n2++)
						{
							if (yF_best[k_Sj][n2] > lp_tolerance2)
							{
									Ctime_Sj += nbQ[n2];
									BDcut_Sj += yF[k_Sj][n2];
									count_Sj++;
						...
						if ( Ctime_Sj + task_Qi > UB)//加cut
						{
							add(BDcut_Sj  <=  count_Sj - 1);
						}
						BDcut_Sj.end();
				}
...
int _tmain(int argc, char* argv[], char* envp[])
{
	...
	//cout<<endl<<"number of constraints = "<<cplex.getNrows()<<endl;
			...
			//cplex.use(IncumbentCallback(env, yF, xF1, xF2, nbQ, nbLocation, nbprecR, subcplex));
				...
				{
					UB = UB_F;
					if (finish_MP - start_MP >= time_left)
