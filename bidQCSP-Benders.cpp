...
bool MPmodel(IloModel model, IloNum nbs, IloIntArray  nbb, IloNumArray nbQ, IloIntArray  nbreadyT,
	...
	if (CF_best < UB)// not converged,algorithm continue
	{
		...
		if (Split_ind2 == 0)
		{
			...
			if (obj == 0 || obj > UB)
			{
				...
				int indicy[nbCrane][nbTask];
				for (int k2 = 0; k2 < nbCrane; k2++)
					for (int i2 = 0; i2 < nbTask; i2++)
						indicy[k2][i2] = 0;

				IloExpr BDcut2(masterEnv);
				for (int i2 = 0; i2 < nbTask - 1; i2++)
						...
						for (int j2 = i2 + 1; j2 < nbTask; j2++)
							if (nbprecR[i2][j2] == 1)
							{
								...
								if (ki != kj && ki >= 0 && kj >= 0)
								{
									if (indicy[ki][i2] == 0)
									{
										BDcut2 += (obj - LB0) * (1 - yF[ki][i2]);
										indicy[ki][i2] = 1;
									}
									if (indicy[kj][j2] == 0)
									{
										BDcut2 += (obj - LB0) * (1 - yF[kj][j2]);
										indicy[kj][j2] = 1;
									}
								}
...
int _tmain(int argc, char* argv[], char* envp[])
{
	...
	for (int my = start_instance; my <= instance_no; my++)
	{
		...
		try
		{
			...
			MPmodel(model, nbs, nbb, nbQ, nbreadyT,
	...
	IloNum* ObjVal, IloNum* UB_Fun, IloIntArray  nbLocation, BoolMatrix nbprecR, NumVarMatrix xF1, NumVarMatrix xF2)
{
	...
	IloNumVarArray maxMovWaiF(env, nbCrane, 0, 100);
					...
					for (int j2 = j + 1; j2 < nbTask; j2++)
						if (nbprecR[j][j2] == 1)
						{
							for (k = 1; k < nbCrane; k++)
							{
								IloExpr epa(env);
								for (int k2 = 0; k2 < k; k2++)
									epa += yF[k2][i] + yF[k2][j2];
								model.add(epa +  yF[k][j]<= 2);
								epa.end();
								//model.add(yF[k-1][i] + yF[k][j] + yF[k - 1][j2] <= 2);
								...
								if (k < nbCrane - 1)
								{
									...
									for (int k2 = k+1; k2 < nbCrane; k2++)
										epa2 += yF[k2][j2];
									model.add(epa2 + yF[k][i] <= 2);
									epa2.end();

									model.add(yF[k][i]+yF[k-1][j]+yF[k+1][j2]<=2);
								}
							}
						}
				}
	...
	BoolVarMatrix rcbayF(env, nbCrane);// variable z_{ki}^1 in the model
	for (k = 0; k < nbCrane; k++)
	{
		rcbayF[k] = IloBoolVarArray(env, nbBay);
	}
	...
	for (k = 0; k < nbCrane; k++)
	{
		for (i = 0; i < nbTask; i++)
		{
			model.add(rcbayF[k][nbLocation[i] - 1] - xF2[k][i] >= 0);
		}
	}
	...
	for (k = 0; k < nbCrane; k++)
		for (i = 0; i < nbBay; i++)
		{
			model.add(rcbayF[k][i] - vF[k] <= 0);
		}



	for (k = 1; k < nbCrane; k++)
		for (i = 1 + safe_margin; i < nbBay; i++)
		{
			IloExpr  epa(env);
			epa += rcbayF[k][i];
			for (j = 0; j < i - safe_margin; j++)
				epa -= rcbayF[k - 1][j];
			model.add(epa <= 0);
			epa.end();
		}
	...
	for (k = 0; k < nbCrane; k++)
	{
		//model.add(maxMovWaiF[k] - gammaF[k] >= 0);
		IloExpr  epa(env);
		epa += maxMovWaiF[k];
		for (j = 0; j < nbBay; j++)
			epa -= mF[k][j];
	...
	{
		IloExpr  epa(env);
		epa += gammaF[k]+ QCmove_time * i * rcbayF[k][i]+ QCmove_time * nbBay * (1- rcbayF[k][i]);
		for (int i2 = 0; i2 < nbBay; i2++)
			epa -= QCmove_time * i2 * endCzF[k][i2];
